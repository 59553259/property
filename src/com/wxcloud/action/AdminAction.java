...
import com.wxcloud.pojo.Admin;
import com.wxcloud.service.AdminService;
...
public class AdminAction extends ActionSupport {
	...
	public String adminLogin() { // 管理员用户登录
		...
		request.getSession().setAttribute(Util.LOGIN_ADMINNAME,
				admin.getUsername()); // 标注系统管理员用户名
		id = admin.getId();
	...
	public String adminEdit() {
		...
		if ((stateOpt == null) || (stateOpt.equals(""))
				|| (stateOpt.equals("0"))) {
	...
	public String adminEditSelf() {
		...
		request.getSession().setAttribute(Util.OPTTYPE,
				Util.OPTTYPE_EDITSELFADMIN); // 设置操作类型
		String pass = request.getParameter("password2");
		if ((password == null) || (password.isEmpty())
				|| (password.length() < 4) || (pass == null)
				|| (!password.equals(pass))) {
	...
	public String adminDelete() {
		...
		HttpServletRequest request = (HttpServletRequest) cxt
				.get(ServletActionContext.HTTP_REQUEST);
		request.getSession().setAttribute(Util.OPTTYPE,
				Util.OPTTYPE_DELETEADMIN); // 设置操作类型
